      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: provider.isLoading ? null : () => context.read<PapersProvider>().loadPapers(),
          )
        ],
      ),
      body: Column(
        children: [
          Padding(
            ...
            child: TextField(
              ...
              onChanged: (v) => context.read<PapersProvider>().setSearchQuery(v),
            ),
          ),
            ...
            child: provider.isLoading
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final id = (paper is Map) ? (paper['id']?.toString() ?? '$index') : '$index';
                      return ListTile(
                        title: Text(title),
                        trailing: const Icon(Icons.chevron_right),
                        onTap: () async {
                          await context.read<PapersProvider>().loadPaperDetail(id);
                          if (!mounted) return;
                          Navigator.of(context).pushNamed(PaperDetailScreen.routeName, arguments: id);
                        },
                      );
                    },
                  ),
